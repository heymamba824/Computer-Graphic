...
const char *WINDOWTITLE = { "OpenGL / GLUT Sample -- Xiang Zhu Graphics" };
...
GLuint    BoxList;
GLuint Blade;// object display list
...
Display( )
{
    ...
    if(WhichProjection==INSIDE)
        
        gluLookAt( -0.4, 1.8, -4.9,    -0.4, 1.8, -10.,     0., 1., 0);
    else
    {
        gluLookAt( 15, 0, 20,     0, 0, 1,     0., 1., 0. );
    }
    // rotate the scene:
    if(WhichProjection!=INSIDE){
...
InitLists( )
{
    ...
    for (int i = 0; i < j; i++)
    {
        glVertex3f(20,5+R*cos(2 * PI*i / j), 5+R*sin(2 * PI*i / j));
    }
    glEnd();
